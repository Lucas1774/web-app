...
class ManualRunTest {
    ...
    private static final LocalDate from = LocalDate.of(2025, 8, 4); // inclusive
    ...
    void assertRecommendationsPrecision() {
        ...
        Map<Recommendation, MarketData> recommendationToFutureMd = recommendationsRepository
                ...
                .filter(r -> isTradingDate(r.getDate())
                        ...
                        && r.getConfidence().compareTo(GROK_FINE_GRAIN_THRESHOLD) >= 0
                )
        ...
        System.out.printf(
                "Sample size: %d%n" +
                        ...
                        "Profitable records (>1%%): %d%n" +
                        "Safe records (loss <1%%): %d%n",
                stats.count,
