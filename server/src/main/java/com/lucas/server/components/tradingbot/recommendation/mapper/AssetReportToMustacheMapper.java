...
public class AssetReportToMustacheMapper implements Mapper<Set<AssetReportRaw>, String> {
    ...
    public String map(Set<AssetReportRaw> assets) throws MappingException {
        ...
        try {
            mustache.execute(out, Collections.singletonMap("assets", assets.stream().map(AssetReport::from).toList()))
                    .flush();
            return out.toString();
        } catch (Exception e) {
    ...
    public record AssetReportRaw(String symbol,
                                 BigDecimal position,
    ...
    public record PricePointRaw(LocalDate date,
                                BigDecimal open,
                                BigDecimal high,
                                BigDecimal low,
                                BigDecimal close,
    ...
    public record NewsItemRaw(String headline,
                              String sentiment,
    ...
    public record AssetReport(String symbol,
                              String position,
        ...
        private static AssetReport from(AssetReportRaw report) {
            return new AssetReport(report.symbol,
                    ...
                    null != report.unrealizedPercentPnL ? report.unrealizedPercentPnL.stripTrailingZeros()
                            .toPlainString()
                            .concat("%") : "0%",
                    String.valueOf(report.historyDays),
                    null != report.premarket ? PricePoint.from(report.premarket) : null,
                    report.priceHistory.stream()
                            .map(PricePoint::from)
        ...
        private record PricePoint(String date,
                                  String open,
                                  String high,
                                  String low,
                                  String close,
            ...
            private static PricePoint from(PricePointRaw point) {
                return new PricePoint(null != point.date ? point.date.toString() : NA,
        ...
        private record NewsItem(String headline,
                                String sentiment,
                                String sentimentConfidence,
                                String summary,
                                String date) {
            private static NewsItem from(NewsItemRaw news) {
                return new NewsItem(news.headline,
                        null != news.sentiment ? "Sentiment: ".concat(news.sentiment).concat(". ") : EMPTY_STRING,
                        null != news.sentimentConfidence
                                ? "Confidence: ".concat(news.sentimentConfidence.stripTrailingZeros().toPlainString())
                                .concat("%. ")
                                : EMPTY_STRING,
                        news.summary,
